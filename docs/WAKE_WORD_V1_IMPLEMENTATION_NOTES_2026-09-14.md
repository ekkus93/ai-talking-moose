...
- Real KWS qualification uses an ephemeral, reproducible 16 kHz mono PCM corpus generated on the pinned macOS runner from built-in English voices. The corpus covers multiple voices/rates, immediate-command speech, gain/noise variants, ordinary speech, `Moose`, `Hey Bruce`, `Hey Moosey`, phonetic near-misses, `moose` in a sentence, and background-speech-style negatives. Generated audio is not committed or logged.
- Linux x86_64 and macOS arm64 real-KWS jobs consume the same generated corpus, verify pinned model/runtime hashes and native architecture before inference, require the one-thread production policy, and execute inference with networking denied. Their retained reports contain only detection and performance metadata, never PCM.
- Runtime diagnostics now record trigger-to-command-handoff latency plus pre-roll replay sample count/duration without recording audio. Deterministic unit acceptance exercises 100 trigger → handoff → suspend → resume cycles while asserting fixed ring capacity, exact trigger count, zero queue drops, recoverable listening state, and clean shutdown.
- Real-model acceptance records initialization latency, baseline and peak RSS with RSS delta, corpus inference CPU time/real-time ratio, and a 30-second silence-equivalent KWS CPU/RTF baseline. These metrics are regression evidence for the tested CI hosts, not performance guarantees for other machines.
